...
class ContractForm(ModelForm):
    ...
    def clean_total_sum(self):
        ...
        # Проверяем
        if total_sum < Decimal(self.data['material_sum'] or 0) + Decimal(self.data['work_sum'] or 0):
    ...
    def clean_prepaid(self):
        ...
        # Проверяем
        if prepaid_sum > Decimal(self.data['total_sum'] or 0):
...
class PaymentForm(ModelForm):
    ...
    def clean_total_sum(self):
        ...
        # Проверяем
        if total_sum < Decimal(self.data['prepaid_sum'] or 0) + Decimal(self.data['retention_sum'] or 0):
            raise ValidationError('Общая сумма не может быть меньше сумм аванса и возврата удержаний')

        return total_sum

    def clean_prepaid_sum(self):
        return self.cleaned_data['total_sum'] or 0
